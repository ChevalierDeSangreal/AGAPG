...
from aerial_gym.envs.base.quad_dynamics_base import Dynamics
...
def load_prepare_trajectory(base_dir, dt, speed_factor, index, test=False):
    """
    ...
    if index > len(data_list):
        ...
        warnings.warn("The sample will be overused!", RuntimeWarning)
    # rand_traj = np.random.choice(data_list)
    rand_traj = data_list[index]
    ...
    # print(take_every_nth, dt / 0.01 * speed_factor)
...
def make_dataset(config=None):
    ...
    # the arena bounds
    # should be equal to space bound in aerial_robot_config.py
    arena_bound_max = np.array([10, 10, 10])  # np.array([8.0, 5.0, 5.0]) #
    arena_bound_min = np.array([-10, -10, 0])
