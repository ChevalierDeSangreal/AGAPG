...
from aerial_gym.utils import task_registry, acc_loss
...
def get_args():
    custom_parameters = [
        ...
        {"name": "--experiment_name", "type": str, "default": "exp4_4__len_sample_150", "help": "Name of the experiment to run or load."},
        ...
        # train setting
        {"name": "--learning_rate", "type":float, "default": 1e-6,
        ...
        {"name": "--gamma", "type":int, "default": 0.9,
        ...
        {"name": "--param_save_path_track_simple", "type":str, "default": '/home/cgv841/wzm/FYP/AGAPG/aerial_gym/param_saved/track_groundVer6.pth',
...
if __name__ == "__main__":
    ...
    for epoch in range(args.num_epoch):
        ...
        for step in range(args.len_sample):
            ...
            loss1 = acc_loss(now_quad_state, tar_pos, criterion)
            loss2 = torch.sqrt(torch.sum((new_state_dyn[:, 2] - 5) ** 2)) / args.batch_size
            loss = loss1
            sum_loss += loss.item()
            loss.backward()
            ...
            # #     # print(action)
            # #     # print(now_state)
                    print(f"    Step {step}:loss = {loss}, horizon_distance = {hoz_dis}, tar_pos = {tar_pos[0]}, new_pos = {new_state_dyn[0, :3]}, action = {action[0]}")
        ...
        scheduler.step()
        print(f"Epoch {epoch}: loss = {loss}, loss2 = {loss2}, hoz_dis = {hoz_dis}, loss_acg = {sum_loss / args.len_sample}")
        writer.add_scalar('Loss', loss.item(), epoch)
        writer.add_scalar('Horizon Distance', hoz_dis.item(), epoch)
        writer.add_scalar('Loss_Avg', sum_loss / args.len_sample, epoch)
