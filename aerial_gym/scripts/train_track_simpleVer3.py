...
def get_args():
    custom_parameters = [
        ...
        {"name": "--experiment_name", "type": str, "default": "exp3", "help": "Name of the experiment to run or load."},
        ...
        {"name": "--param_save_path_track_simple", "type":str, "default": '/home/cgv841/wzm/FYP/AGAPG/aerial_gym/param_saved/track_simpleVer7.pth',
            "help": "The path to model parameters"},
        {"name": "--param_load_path_track_simple", "type":str, "default": '/home/cgv841/wzm/FYP/AGAPG/aerial_gym/param_saved/track_simpleVer2.pth',
            "help": "The path to model parameters"},
...
if __name__ == "__main__":
    ...
    for epoch in range(args.num_epoch):
        ...
        # if 1:
            ...
            tmp_loss = criterion(new_state_dyn[:, :3], tar_pos)
            if (epoch + 1) % 10 == 0:
                if (step + 1) % 10 == 0:
            # #     # print(action)
            # #     # print(now_state)
                    print(f"    Step {step}: loss = {tmp_loss}, tar_pos = {tar_pos[0]}, now_pos = {now_state[0, :3]}, new_pos = {new_state_dyn[0, :3]}, action = {action[0]}")
        ...
        loss = criterion(new_state_dyn[:, :2], tar_pos[:, :2]) + torch.sqrt(torch.sum((new_state_dyn[:, 3] - 5)) ** 2) / args.batch_size
        print(f"Epoch {epoch}: loss = {loss}, loss2 = {torch.sqrt(torch.sum((new_state_dyn[:, 3] - 5)) ** 2) / args.batch_size}")
        writer.add_scalar('Loss', loss.item(), epoch)
        writer.add_scalar('Loss_dis', torch.sqrt(torch.sum((new_state_dyn[:, 3] - 5)) ** 2) / args.batch_size, epoch)
        loss.backward()
        # dis.backward()
