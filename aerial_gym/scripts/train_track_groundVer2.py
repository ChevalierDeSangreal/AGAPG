""" 
    You have to modify cuda:0 to cuda:x in
    dynamics.py Dynamic class and line around 111
    gutils.py line 305
"""
...
def get_args():
    custom_parameters = [
        ...
        {"name": "--experiment_name", "type": str, "default": "exp3_3__lr_2e-3", "help": "Name of the experiment to run or load."},
...
if __name__ == "__main__":
    ...
    # print("device_id", args.compute_device_id)
    ...
    for epoch in range(args.num_epoch):
        ...
        # if 1:
            ...
            # print("Image cuda:", image.device)
            action = model(now_quad_state, image)
            
            # action = torch_rand_float(-1.0, 1.0, (12, 12), device)
            # print(action, now_state)
            # print('Here I am!!!')
            new_state_dyn = dynamic(now_quad_state, action, envs.cfg.sim.dt)
            # new_state_dyn = dynamic(now_quad_state, action)
            # print(new_state_dyn.device, action.device)
            new_state_sim, tar_state = envs.step(action)
            tar_pos = tar_state[:, :3]
            # print(new_state_dyn, tar_pos)
            # tmp = torch.zeros_like(new_state_dyn).to(device, dtype=torch.float32)
            now_quad_state = new_state_dyn
            loss1 = criterion(new_state_dyn[:, :2], tar_pos[:, :2])
            # loss1 = criterion(new_state_dyn[:, :2], tmp[:, :2])
            # loss2 = torch.sqrt(torch.sum((new_state_dyn[:, 3] - 5) ** 2)) / args.batch_size
            loss =  loss1
            # print(loss.device, new_state_dyn.device, tar_pos.device)
            if step == args.len_sample - 1:
                loss.backward()
            else:
        ...
        # loss = criterion(new_state_dyn[:, :2], tmp[:, :2])
        # net_plot = make_dot(loss, params = dict(model.named_parameters()))
        # net_plot.view()
        # loss.backward()
        # optimizer.step()
        ...
        # writer.add_scalar('Loss_dis', loss2.item(), epoch)
        # break
