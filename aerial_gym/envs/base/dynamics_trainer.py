...
from aerial_gym.envs.base.dynamics_learnt import LearntDynamics

def qua_generator(batch_size, device):
    # 生成随机张量，形状为 (batch_size, 4)
    shape = (batch_size, 3)
    uniform_random = torch.rand(shape)
    tensor_uniform_pi = 2 * uniform_random - 1
    tensor_uniform_pi *= torch.tensor([torch.pi, torch.pi, torch.pi])
    tensor_uniform_pi = tensor_uniform_pi.to(device)
    return axis_angle_to_quaternion(tensor_uniform_pi)


    

class DynamicsTrainer(BaseTask):
    ...
    def __init__(self, cfg: AerialRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.trained_dynamics = LearntDynamics(self.device, self.cfg.other.param_file_path)
        self.trained_dynamics.load_parameters()
        self.trained_dynamics.to(self.device)
    ...
    def step(self, actions):
        ...
        old_state = torch.zeros((self.num_envs, 12)).to(self.device)
        old_state[:, :3] = obs[:, :3] # position
        old_state[:, 3:6] = self.qua2euler(obs[:, 3:7]) # orientation
        old_state[:, 6:9] = obs[:, 7:10] # linear acceleration
        old_state[:, 9:12] = obs[:, 10:13] # angular acceleration
        

        # new state generated by trained dynamics
        # print(old_state, actions)
        new_state_td = self.trained_dynamics(old_state, actions, self.cfg.sim.dt)
        ...
        return new_state_ag, new_state_td
    ...
    def reset_idx(self, env_ids):
        ...
        self.root_states[env_ids,
                         0:3] = 10.0*torch_rand_float(-1.0, 1.0, (num_resets, 3), self.device)
        self.root_states[env_ids,
                         7:10] = 10.0*torch_rand_float(-1.0, 1.0, (num_resets, 3), self.device)
        self.root_states[env_ids,
                         10:13] = 10.0*torch_rand_float(-1.0, 1.0, (num_resets, 3), self.device)

        self.root_states[env_ids, 3:7] = qua_generator(num_resets, self.device)
